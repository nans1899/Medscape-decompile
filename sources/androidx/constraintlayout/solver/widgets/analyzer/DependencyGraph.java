...
public class DependencyGraph {
    ...
    private String nodeDefinition(WidgetRun widgetRun) {
        ...
        String str9 = str8 + HtmlObject.HtmlMarkUp.CLOSE_BRACKER + debugName + str3 + " </TD>";
        if (!z) {
        ...
        if ((widgetRun instanceof HorizontalWidgetRun) || (((z = widgetRun instanceof ChainRun)) && ((ChainRun) widgetRun).orientation == 0)) {
            ...
            if (horizontalDimensionBehaviour == ConstraintWidget.DimensionBehaviour.FIXED || horizontalDimensionBehaviour == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT) {
                if (!dependencyNode.targets.isEmpty() && dependencyNode2.targets.isEmpty()) {
                    generateDisplayNode = generateDisplayNode + (IOUtils.LINE_SEPARATOR_UNIX + dependencyNode2.name() + " -> " + dependencyNode.name() + IOUtils.LINE_SEPARATOR_UNIX);
                } else if (dependencyNode.targets.isEmpty() && !dependencyNode2.targets.isEmpty()) {
                    generateDisplayNode = generateDisplayNode + (IOUtils.LINE_SEPARATOR_UNIX + dependencyNode.name() + " -> " + dependencyNode2.name() + IOUtils.LINE_SEPARATOR_UNIX);
                }
