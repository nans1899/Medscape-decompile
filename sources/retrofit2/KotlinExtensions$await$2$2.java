    ...
    public void onResponse(Call<T> call, Response<T> response) {
        ...
        if (response.isSuccessful()) {
            ...
            if (body == null) {
                ...
                this.$continuation.resumeWith(Result.m6constructorimpl(ResultKt.createFailure(kotlinNullPointerException)));
                return;
            }
            Result.Companion companion2 = Result.Companion;
            this.$continuation.resumeWith(Result.m6constructorimpl(body));
            return;
        }
        Result.Companion companion3 = Result.Companion;
        this.$continuation.resumeWith(Result.m6constructorimpl(ResultKt.createFailure(new HttpException(response))));
    }

    public void onFailure(Call<T> call, Throwable th) {
        Intrinsics.checkParameterIsNotNull(call, NotificationCompat.CATEGORY_CALL);
        Intrinsics.checkParameterIsNotNull(th, Constants.APPBOY_PUSH_TITLE_KEY);
        Result.Companion companion = Result.Companion;
        this.$continuation.resumeWith(Result.m6constructorimpl(ResultKt.createFailure(th)));
    }
}
