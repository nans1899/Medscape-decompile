...
public class HurlStack implements HttpStack {
        ...
        if (openConnection.getResponseCode() != -1) {
            ...
            for (Map.Entry entry : openConnection.getHeaderFields().entrySet()) {
                if (entry.getKey() != null) {
                    for (String basicHeader : (List) entry.getValue()) {
                        basicHttpResponse.addHeader(new BasicHeader((String) entry.getKey(), basicHeader));
                    }
                }
            }
    ...
    private HttpURLConnection openConnection(URL url, Request<?> request) throws IOException {
        ...
        if (RequestBuilders.DEFAULT_SCHEME.equals(url.getProtocol()) && (sSLSocketFactory = this.mSslSocketFactory) != null) {
