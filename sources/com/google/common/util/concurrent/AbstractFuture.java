...
public abstract class AbstractFuture<V> extends FluentFuture<V> {
    ...
    public V get(long j, TimeUnit timeUnit) throws InterruptedException, TimeoutException, ExecutionException {
        ...
        if (!Thread.interrupted()) {
            ...
            if (isDone()) {
                throw new TimeoutException("Waited " + j2 + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + timeUnit.toString().toLowerCase(Locale.ROOT) + " but future completed as timeout expired");
            }
            throw new TimeoutException("Waited " + j2 + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + timeUnit.toString().toLowerCase(Locale.ROOT) + " for " + abstractFuture);
        }
        throw new InterruptedException();
    }
