...
public final class VCardResultParser extends ResultParser {
    ...
    public AddressBookParsedResult parse(Result result) {
        ...
        List<List<String>> matchVCardPrefixedField5 = matchVCardPrefixedField(FeedDetail.F_URL, massagedText, true, false);
            ...
            if (indexOf < 0) {
                ...
                if (indexOf > end) {
                    ...
                    if (z3) {
                        ...
                        if (z2) {
                            str3 = UNESCAPED_SEMICOLONS.matcher(str3).replaceAll(IOUtils.LINE_SEPARATOR_UNIX).trim();
                        }
                    } else {
                        if (z2) {
                            substring = UNESCAPED_SEMICOLONS.matcher(substring).replaceAll(IOUtils.LINE_SEPARATOR_UNIX).trim();
                        }
                        str3 = VCARD_ESCAPES.matcher(NEWLINE_ESCAPE.matcher(CR_LF_SPACE_TAB.matcher(substring).replaceAll("")).replaceAll(IOUtils.LINE_SEPARATOR_UNIX)).replaceAll("$1");
                    }
                    if (arrayList == null) {
